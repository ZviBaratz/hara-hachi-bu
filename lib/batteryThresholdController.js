...
export const BatteryThresholdController = GObject.registerClass(
    {
        GTypeName: 'HhbBatteryThresholdController',
        Signals: {
    ...
    class BatteryThresholdController extends GObject.Object {
        constructor(settings) {
            ...
            // Reentrancy guard for _updateAutoDischarge
            this._updatingAutoDischarge = false;
        ...
        async initialize(_extensionObject) {
            ...
            this._settings.set_boolean('device-has-start-threshold', this._device?.hasStartThreshold ?? false);
            ...
            if (this._device) {
                ...
                const signalHandlers = [
                    ...
                    (dev, enabled) => {
                        ...
                        if (!enabled && this._autoManagementActive) this._autoManagementActive = false;
                    },
                ];
                ...
                if (GObject.signal_lookup('partial-failure', this._device.constructor.$gtype)) {
                    signalHandlers.push('partial-failure', (dev, succeededBat, failedBats) => {
                ...
                console.debug('Hara Hachi Bu: No supported device found');
                // Fallback paths for basic monitoring without threshold control
                const bat1Exists = Helper.fileExists(
                    `${Constants.SYSFS_POWER_SUPPLY_PATH}/BAT1/${Constants.CAPACITY_FILE}`
                );
                const batteryName =
                    bat1Exists &&
                    !Helper.fileExists(`${Constants.SYSFS_POWER_SUPPLY_PATH}/BAT0/${Constants.CAPACITY_FILE}`)
                        ? 'BAT1'
                        : 'BAT0';
            ...
            if (this._batteryLevel === null || this._batteryLevel === undefined) {
                if (this._device && typeof this._device.getBatteryLevel === 'function')
                    this._batteryLevel = this._device.getBatteryLevel();
                else this._batteryLevel = (await readFileIntAsync(this._capacityPath)) ?? 0;
            }
        ...
        <node>
          <interface name="org.freedesktop.UPower">
            ...
            const acOnline = await Helper.getAcOnlineSysfs(Constants.SYSFS_POWER_SUPPLY_PATH, () => this._destroyed);
            ...
            return new Promise((resolve) => {
                ...
                this._upowerProxy = new upowerProxyWrapper(
                    ...
                    (proxy, error) => {
                        ...
                        if (error) {
                            console.error(`Hara Hachi Bu: Failed to create main UPower proxy: ${error}`);
                        } else {
                        ...
                        if (error) {
                            ...
                            this._proxy.connectObject(
                                ...
                                (p, changed) => {
                                    ...
                                    if (statusChanged) this.emit('battery-status-changed');
                                },
                                this
                            );
                        }
        ...
        _initializeStatusMonitoring() {
            ...
            try {
                ...
                this._monitorStatus.connectObject(
                    'changed',
                    (obj, file, other, eventType) => {
        ...
        get batteryHealth() {
            if (this._device && typeof this._device.getHealth === 'function') return this._device.getHealth();
        ...
        setThresholds(startValue, endValue) {
        ...
        async setForceDischarge(enabled, isManual = true) {
            if (!this.supportsForceDischarge) return false;

            let success;
            try {
                success = await this._device.setForceDischarge(enabled);
            } catch (e) {
                if (!this._destroyed) console.error(`Hara Hachi Bu: Error setting force discharge: ${e.message}`);
                return false;
            }

            if (isManual) {
                console.debug('Hara Hachi Bu: User manually toggled force discharge, stopping auto-management');
                this._autoManagementActive = false;

                if (this._settings.get_boolean('auto-manage-battery-levels')) {
                    console.debug('Hara Hachi Bu: Disabling auto-management due to manual override');
        ...
        async _updateAutoDischarge() {
            if (!this.canControlThresholds || !this.supportsForceDischarge || this._destroyed) return;

            if (this._updatingAutoDischarge) {
                this._pendingAutoDischargeUpdate = true;
                return;
            }

            this._updatingAutoDischarge = true;
            try {
                ...
                if (this._onBattery) {
                    if (this.forceDischargeEnabled) {
                        console.debug('Hara Hachi Bu: Auto-discharge: On battery, disabling force discharge');
                        const success = await this.setForceDischarge(false, false);
                        if (!success && !this._destroyed)
                            console.debug('Hara Hachi Bu: Auto-discharge disable failed (emergency stop on battery)');
                ...
                // On end-only devices, startThreshold is -1 (no start threshold file).
                // Use endThreshold minus a margin as the stop point instead.
                const effectiveStopThreshold =
                    startThreshold >= 0 ? startThreshold : endThreshold - AUTO_DISCHARGE_STOP_MARGIN;
                ...
                if (batteryLevel > endThreshold + AUTO_DISCHARGE_HYSTERESIS && !this.forceDischargeEnabled) {
                    console.debug(
                        `Hara Hachi Bu: Auto-discharge: Level ${batteryLevel}% > End ${endThreshold}% + ${AUTO_DISCHARGE_HYSTERESIS}%, enabling`
                    );
                    ...
                    if (success) {
                        ...
                        console.debug(
                            `Hara Hachi Bu: Auto-discharge enable failed (battery: ${batteryLevel}%, threshold: ${endThreshold}%)`
                        );
                    }
                    ...
                    console.debug(
                        `Hara Hachi Bu: Auto-discharge: Level ${batteryLevel}% <= Stop ${effectiveStopThreshold}%, disabling`
                    );
                    const success = await this.setForceDischarge(false, false);
                    if (success) {
                        this._autoManagementActive = false;
                        ...
                        console.debug(
                            `Hara Hachi Bu: Auto-discharge disable failed (battery: ${batteryLevel}%, threshold: ${effectiveStopThreshold}%)`
                        );
                    }
                }
            } finally {
        ...
        async refreshValues() {
            if (this._device && typeof this._device.refreshValues === 'function') await this._device.refreshValues();

            // Update battery level from device if available, otherwise read from path
            if (this._device && typeof this._device.getBatteryLevel === 'function') {
                const level = this._device.getBatteryLevel();
                if (level !== null && level !== undefined) this._batteryLevel = level;
            } else {
