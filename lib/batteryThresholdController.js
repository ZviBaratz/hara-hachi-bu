...
export const BatteryThresholdController = GObject.registerClass({
    ...
    async _initializeBatteryMonitoring() {
        ...
        try {
            ...
            // Check multiple known AC adapter names for compatibility
            const acAdapterNames = ['AC', 'ACAD', 'ADP0', 'ADP1'];
            let acOnline = null;

            for (const name of acAdapterNames) {
                const path = `/sys/class/power_supply/${name}/online`;
