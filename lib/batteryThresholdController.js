...
const {readFile, readFileInt} = Helper;
    ...
    async _initializeBatteryMonitoring() {
        ...
        try {
            ...
            try {
                ...
                const enumerator = psDir.enumerate_children('standard::name', Gio.FileQueryInfoFlags.NONE, null);
                let info;
                while ((info = enumerator.next_file(null)) !== null) {
                    const name = info.get_name();
                    const type = readFile(`/sys/class/power_supply/${name}/type`);
                    if (type === 'Mains') {
                        const online = readFile(`/sys/class/power_supply/${name}/online`);
                        if (online !== null) {
                            acOnline = online;
                            break;
                        }
