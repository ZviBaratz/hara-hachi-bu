            ...
            const signalHandlers = [
                ...
                'force-discharge-changed', (dev, enabled) => {
                    ...
                    this._updateSysfsStatus().catch(e => {
                        if (!this._destroyed)
                            console.error(`Unified Power Manager: sysfs status update error: ${e.message}`);
                    });
    ...
    async _updateAutoDischarge() {
        ...
        try {
            ...
            // 3. Stop Condition: At or Below Effective Stop Threshold
                ...
                console.debug(`Unified Power Manager: Auto-discharge: Level ${batteryLevel}% <= Stop ${effectiveStopThreshold}%, disabling`);
                const success = await this.setForceDischarge(false, false);
                if (success) {
                    this._autoManagementActive = false;
                } else if (!this._destroyed) {
                    console.debug(`Unified Power Manager: Auto-discharge disable failed (battery: ${batteryLevel}%, threshold: ${effectiveStopThreshold}%)`);
                }
            }
        } finally {
            this._updatingAutoDischarge = false;
