...
export const BatteryThresholdController = GObject.registerClass({
    ...
    async initialize(extensionObject) {
        ...
        // Initialize battery monitoring for status changes
                    ...
                    if (this._batteryLevel !== batteryLevel) {
                        ...
                        if (this._forceDischargeEnabled && this._batteryLevel <= this.endLimitValue) {
                            this.setForceDischarge(false).catch(e => {
                                console.error('UPM: Failed to auto-disable force discharge:', e);
                            });
                        }
                    }
    ...
    get isAvailable() {
        return this._initialized && !this._missingHelper;
    }
    ...
    async setThresholds(startValue, endValue) {
        if (!this._initialized || this._missingHelper)
            return false;
    ...
    async setForceDischarge(enabled) {
        if (!this._supportsForceDischarge || !this._initialized || this._missingHelper)
            return false;
