...
export const BatteryThresholdController = GObject.registerClass({
    ...
    getBatteryStatus() {
        // Check sysfs first for "Not charging" (threshold inhibited)
        const sysfsStatus = readFile(this._statusPath);
        if (sysfsStatus === 'Not charging') {
            return 'Not charging';  // Charging inhibited by threshold
        }

        // Fall back to UPower states for standard statuses
        if (this._batteryState === 1) return 'Charging';
        if (this._batteryState === 2) return 'Discharging';
        if (this._batteryState === 4) return 'Full';

        // Final fallback: use sysfs for other statuses
    ...
    async setForceDischarge(enabled) {
        ...
        // Detect if this is a user-initiated call
        const isUserInitiated = !this._autoEnableInProgress && !this._forceDischargeDisableInProgress;
