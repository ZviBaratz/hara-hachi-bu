...
export const BatteryThresholdController = GObject.registerClass({
    ...
    async _updateSysfsStatus() {
        try {
            ...
            if (!this._destroyed)
                console.error(`Hara Hachi Bu: Error reading status: ${e.message}`);
        }
    }
    ...
    async setForceDischarge(enabled, isManual = true) {
        ...
        try {
            ...
            if (!this._destroyed)
                console.error(`Hara Hachi Bu: Error setting force discharge: ${e.message}`);
            return false;
        }
    ...
    async _updateAutoDischarge() {
        ...
        try {
            ...
            const effectiveStopThreshold = startThreshold >= 0
                ? startThreshold
                : endThreshold - AUTO_DISCHARGE_STOP_MARGIN;
            ...
            if (batteryLevel > (endThreshold + AUTO_DISCHARGE_HYSTERESIS) && !this.forceDischargeEnabled) {
                console.debug(`Hara Hachi Bu: Auto-discharge: Level ${batteryLevel}% > End ${endThreshold}% + ${AUTO_DISCHARGE_HYSTERESIS}%, enabling`);
                const success = await this.setForceDischarge(true, false);
                if (success) {
                    this._autoManagementActive = true;
                } else if (!this._destroyed) {
                    console.debug(`Hara Hachi Bu: Auto-discharge enable failed (battery: ${batteryLevel}%, threshold: ${endThreshold}%)`);
                }
            }
            // 3. Stop Condition: At or Below Effective Stop Threshold
                ...
                console.debug(`Hara Hachi Bu: Auto-discharge: Level ${batteryLevel}% <= Stop ${effectiveStopThreshold}%, disabling`);
                const success = await this.setForceDischarge(false, false);
                if (success) {
                    this._autoManagementActive = false;
                } else if (!this._destroyed) {
                    console.debug(`Hara Hachi Bu: Auto-discharge disable failed (battery: ${batteryLevel}%, threshold: ${effectiveStopThreshold}%)`);
                }
            }
        } finally {
            this._updatingAutoDischarge = false;
