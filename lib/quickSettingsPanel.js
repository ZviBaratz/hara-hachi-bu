...
const {BATTERY_MODES, POWER_MODES, hasAutoRules} = ProfileMatcher;
...
class PowerManagerToggle extends QuickSettings.QuickMenuToggle {
    constructor(settings, extensionObject, stateManager) {
        ...
        // Add content to the scroll box - Profiles section (expanded by default)
    ...
    _rebuildProfileItems() {
        ...
        for (const profile of profiles) {
            ...
            const hasRules = hasAutoRules(profile);
    ...
    _buildBatteryModeSection() {
        ...
        if (this._stateManager.batteryNeedsHelper) {
            const item = new PopupMenu.PopupMenuItem(_('Battery Control: Install Helper Script'));
            item.connectObject('activate', () => {
                ...
                // Also show notification for users who closed the menu quickly
                Main.notify(
    ...
    _addForceDischargeToggle() {
        ...
        this._forceDischargeItem.connectObject('toggled', (o, state) => {
            // Manual toggle pauses auto-management
            this._stateManager.pauseAutoManage();
    ...
    _updateForceDischargeState() {
        ...
        // Disable toggle when on battery (force discharge makes no sense on battery)
        this._forceDischargeItem.sensitive = !onBattery;

        // Update tooltip/subtitle
        if (onBattery) {
            this._forceDischargeItem.label.text = _('Force Discharge (unavailable on battery)');
        } else {
            this._forceDischargeItem.label.text = _('Force Discharge');
        }
    }
    ...
    _updatePausedIndicator() {
        ...
        // Show paused indicator only if auto-switch is enabled AND currently paused
        this._pausedIndicator.actor.visible = autoSwitchEnabled && paused;
    }
    ...
    _addBatteryHealthDisplay() {
        ...
        this._batteryHealthItem = new PopupMenu.PopupMenuItem(
            _('Battery Health: %d%%').format(health),
            { reactive: false }
        );
    ...
    _updateBatteryHealthDisplay() {
        ...
        this._batteryHealthItem.label.text = _('Battery Health: %d%%').format(clampedHealth);
    ...
    _updateProfileOrnaments() {
        ...
        if (this._customProfileItem) {
            ...
            this._customProfileItem.setOrnament(isCustom
                ? PopupMenu.Ornament.CHECK
                : PopupMenu.Ornament.NONE);
            this._customProfileItem.actor.visible = isCustom;
        }
    }
    ...
    _setLoadingState(loading, message = null) {
        ...
        if (loading && message) {
            ...
            if (this.subtitleLabel) {
                this.subtitleLabel.add_style_class_name('loading-state');
            }
            ...
            if (this.subtitleLabel) {
                this.subtitleLabel.remove_style_class_name('loading-state');
            }
