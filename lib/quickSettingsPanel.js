...
class PowerManagerToggle extends QuickSettings.QuickMenuToggle {
        ...
        if (this._stateManager.batteryNeedsHelper) {
            ...
            item.connectObject('activate', () => {
                ...
                Main.notify(
                    ...
                    _('Run "sudo ./install-helper.sh" in the extension directory to enable battery control.')
                );
            }, this);
    ...
    _updateIcon() {
        ...
        if (batteryStatus === 'Charging') {
            headerSubtitle += _(' \u2022 Charging');
        } else if (batteryStatus === 'Discharging') {
            if (this._stateManager.forceDischargeEnabled)
                headerSubtitle += _(' \u2022 Force discharging');
            else
                headerSubtitle += _(' \u2022 Discharging');
        } else if (batteryStatus === 'Not charging') {
            headerSubtitle += _(' \u2022 Charging inhibited');
        } else if (batteryStatus === 'Full') {
            headerSubtitle += _(' \u2022 Full');
        }

        this.menu.setHeader(this.gicon, _('Power'), headerSubtitle);
    }
