...
class PowerManagerToggle extends QuickSettings.QuickMenuToggle {
    ...
    _buildBatteryModeItems() {
        ...
        for (const mode of batteryModes) {
            ...
            const label = `${_(config.label)} (${range}) — ${_(config.description)}`;
    ...
    _addAutoManageSection() {
        ...
        this._pausedIndicator = new PopupMenu.PopupMenuItem(_('Auto-switching paused'));
    ...
    _getBatteryStatusText() {
        ...
        if (currentProfile) {
            ...
            // No profile matched - manual mode
            parts.push(_('Manual'));
        }
        ...
        if (this._stateManager.boostChargeActive) {
            ...
            if (remaining > 0) {
                const remainingText = this._formatTime(remaining);
                parts.push(_('Boost Charge (%s remaining)').format(remainingText));
            } else {
