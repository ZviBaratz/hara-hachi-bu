...
class PowerManagerToggle extends QuickSettings.QuickMenuToggle {
    ...
    _updateProfileOrnaments() {
        ...
        for (const [profile, item] of Object.entries(this._profileItems)) {
            item.setOrnament(profile === currentProfile ?
                PopupMenu.Ornament.CHECK : PopupMenu.Ornament.NONE);
        }
    }

    _updatePowerModeOrnaments() {
        const currentMode = this._stateManager.currentPowerMode;
        for (const [mode, item] of Object.entries(this._powerModeItems)) {
            item.setOrnament(mode === currentMode ?
                PopupMenu.Ornament.CHECK : PopupMenu.Ornament.NONE);
        }
    }

    _updateBatteryModeOrnaments() {
        const currentMode = this._stateManager.currentBatteryMode;
        for (const [mode, item] of Object.entries(this._batteryModeItems)) {
            item.setOrnament(mode === currentMode ?
                PopupMenu.Ornament.CHECK : PopupMenu.Ornament.NONE);
        }
    }
