...
const {BATTERY_MODES, POWER_MODES, isAutoManaged, getProfileDisplayName} = ProfileMatcher;
...
class PowerManagerToggle extends QuickSettings.QuickMenuToggle {
    constructor(settings, extensionObject, stateManager) {
        ...
        // Boost charge toggle (if battery control is available)
        if (this._stateManager.batteryControlAvailable)
            this._addBoostChargeToggle();
        ...
        this._stateManager.connectObject('boost-charge-changed', () => {
    ...
    _buildErrorSection() {
        ...
        if (this._stateManager.batteryNeedsHelper) {
            ...
            item.connectObject('activate', () => {
                ...
                item.label.add_style_class_name('upm-success');
                ...
                this._clipboardFeedbackTimeoutId = GLib.timeout_add(GLib.PRIORITY_DEFAULT, CLIPBOARD_FEEDBACK_TIMEOUT_MS, () => {
                    ...
                    if (item.label) {
                        ...
                        item.label.remove_style_class_name('upm-success');
                    }
    ...
    _rebuildProfileItems() {
        ...
        if (profiles.length === 0) {
            const hint = new PopupMenu.PopupMenuItem(
                _('Create profiles in Settings to save mode combinations'),
        ...
        for (const profile of profiles) {
            ...
            if (hasRules) {
                ...
                if (badge.accessible)
                    badge.accessible.accessible_name = _('Automatically managed profile');
            ...
            if (item.accessible)
                item.accessible.accessible_name = labelText;
            ...
            item.connectObject('activate', async () => {
                ...
                try {
                    ...
                    if (!success && !this._destroyed) {
                        // StateManager already notified user, just revert UI selection
                        this._updateUI();
                    }
                } finally {
    ...
    _updateManualOverridesExpanded() {
        ...
        if (this._manualOverridesHeader)
            this._manualOverridesHeader.label.text = _('Manual Controls');
    ...
    _buildPowerModeSection() {
        ...
        for (const mode of powerModes) {
            ...
            item.accessible_role = Atk.Role.RADIO_MENU_ITEM;
            if (item.accessible)
                item.accessible.accessible_name = label;
            item.connectObject('activate', async () => {
                // Prevent double-clicks during loading
                if (this._isLoading)
                    return;
    ...
    _buildBatteryModeItems() {
        ...
        for (const mode of batteryModes) {
            ...
            item.accessible_role = Atk.Role.RADIO_MENU_ITEM;
            if (item.accessible)
                item.accessible.accessible_name = `${label} — ${_(config.description)}`;
            item.connectObject('activate', async () => {
                // Prevent double-clicks during loading
                if (this._isLoading)
                    return;
    ...
    _addForceDischargeToggle() {
        ...
        if (this._forceDischargeItem.accessible)
            this._forceDischargeItem.accessible.accessible_name = _('Force Discharge: use battery power while connected to AC');
    ...
    _addAutoManageSection() {
        ...
        if (this._autoSwitchToggle.accessible)
            this._autoSwitchToggle.accessible.accessible_name = _('Auto-switch profiles: automatically apply profiles when system conditions change');
    ...
    _updateBatteryHealthColor(health) {
        ...
        this._batteryHealthItem.label.remove_style_class_name('upm-health-good');
        this._batteryHealthItem.label.remove_style_class_name('upm-health-fair');
        this._batteryHealthItem.label.remove_style_class_name('upm-health-poor');

        // Apply appropriate color and text descriptor based on health level
        let descriptor;
        if (health >= 85) {
            this._batteryHealthItem.label.add_style_class_name('upm-health-good');
            descriptor = _('Good');
        } else if (health >= 70) {
            this._batteryHealthItem.label.add_style_class_name('upm-health-fair');
            descriptor = _('Fair');
        } else {
            this._batteryHealthItem.label.add_style_class_name('upm-health-poor');
            descriptor = _('Poor');
        }
    ...
    _addBoostChargeToggle() {
        ...
        this._boostChargeItem.connectObject('toggled', async () => {
            if (this._isLoading) return;
    ...
    _updateBoostChargeState() {
        ...
        // Show remaining time when boost charge is active
        if (active) {
            const endTime = this._stateManager.boostChargeEndTime;
            if (endTime) {
                const hours = String(endTime.getHours()).padStart(2, '0');
                const minutes = String(endTime.getMinutes()).padStart(2, '0');
                this._boostChargeItem.label.text = _('Boost Charge (until %s)').format(`${hours}:${minutes}`);
            } else {
                this._boostChargeItem.label.text = _('Boost Charge');
            }
        } else {
            this._boostChargeItem.label.text = onBattery
                ? _('Boost Charge (AC only)')
                : _('Boost Charge — temporarily charge to 100%');
        }
    }

    _updateBoostChargeToggle() {
        if (this._boostChargeItem)
            this._boostChargeItem.setToggleState(this._stateManager.boostChargeActive);
    }
    ...
    _getBatteryStatusText() {
        ...
        // Add current profile name (biggest ambient awareness improvement!)
        const currentProfile = this._stateManager.currentProfile;
        const autoSwitchEnabled = this._settings.get_boolean('auto-switch-enabled');
        const paused = this._stateManager.autoManagePaused;
        ...
        if (currentProfile) {
            ...
            if (profile) {
                ...
                // Add paused indicator if auto-switching is enabled but manually overridden
                if (autoSwitchEnabled && paused) {
                    profileText = _('%s (manual)').format(profileText);
                }
                parts.push(profileText);
            }
        } else {
            // No profile matched
            parts.push(_('No profile'));
        }

        parts.push(_('%d%%').format(batteryLevel));

        // Add boost charge indicator with remaining time
        if (this._stateManager.boostChargeActive) {
            const remaining = this._stateManager.boostChargeRemainingSeconds;
            if (remaining > 0) {
                parts.push(_('Boost (%s)').format(this._formatTime(remaining)));
            } else {
                parts.push(_('Boost Charge'));
            }
        }

        // Skip charging status when boost is active (boost implies charging)
        if (batteryStatus === 'Charging' && !this._stateManager.boostChargeActive) {
            const timeToFull = this._stateManager.timeToFull;
            if (timeToFull > 0)
                parts.push(_('Charging (%s)').format(this._formatTime(timeToFull)));
            else
            ...
            if (timeToEmpty > 0) {
                parts.push(isForce
                    ? _('Force discharge (%s)').format(this._formatTime(timeToEmpty))
                    : _('Discharging (%s)').format(this._formatTime(timeToEmpty)));
            } else {
                parts.push(isForce ? _('Force discharge') : _('Discharging'));
            }
        ...
        if (currentProfile && this._profileItems?.[currentProfile]) {
            ...
            if (profileConfig?.schedule?.enabled) {
                ...
                if (endTime) {
                    ...
                    item.label.text = _('%s (until %s)').format(baseName, endTime);

                    // Update accessible name to include schedule end time
                    if (item.accessible) {
                        let name = item.label.text;
                        if (item._isAutoManaged)
                            name += ` (${_('applies automatically')})`;
    ...
    _setLoadingState(loading, message = null) {
        ...
        if (loading && message) {
            ...
            this.add_style_class_name('upm-loading-state');
            this._setMenuItemsSensitive(false);
        } else if (!loading) {
            this._updateSubtitle();
            this.remove_style_class_name('upm-loading-state');
    ...
    _setMenuItemsSensitive(sensitive) {
        ...
        if (this._boostChargeItem) {
            this._boostChargeItem.sensitive = sensitive &&
                (!this._stateManager.onBattery || this._stateManager.boostChargeActive);
        }
