...
class PowerManagerToggle extends QuickSettings.QuickMenuToggle {
    constructor(settings, extensionObject, stateManager) {
        ...
        // Set up menu header
        this.menu.setHeader('power-profile-balanced-symbolic', 'Power');
    ...
    _buildBatteryModeSection() {
        ...
        if (this._stateManager.batteryNeedsHelper) {
            const item = new PopupMenu.PopupMenuItem('Setup Required (Click for Info)');
            item.connectObject('activate', () => {
                Main.panel.closeQuickSettings();
                Main.notify(
                    'Unified Power Manager',
                    'Run "sudo ./install-helper.sh" in the extension directory to enable battery control.'
                );
            }, this);
    ...
    _addForceDischargeToggle() {
        ...
        this._scrollBox.add_child(this._forceDischargeSeperator.actor);
    ...
    _addSettingsShortcut() {
        // Create a section for the settings item
        ...
        // Add items to the section properly
        settingsSection.addMenuItem(separator);
        settingsSection.addMenuItem(settingsItem);

        // Add the section's actor to scrollBox
        this._scrollBox.add_child(settingsSection.actor);
    }
    ...
    _updateSubtitle() {
        // Show battery status in subtitle instead of profile name
    ...
    _updateIcon() {
        ...
        const batteryLevel = this._stateManager.batteryLevel;
        const batteryStatus = this._stateManager.getBatteryStatus();
        let headerSubtitle = `${batteryLevel}%`;

        if (batteryStatus === 'Charging') {
            headerSubtitle += ' \u2022 Charging';
        } else if (batteryStatus === 'Discharging') {
            if (this._stateManager.forceDischargeEnabled)
                headerSubtitle += ' \u2022 Force discharging';
            else
                headerSubtitle += ' \u2022 Discharging';
        } else if (batteryStatus === 'Not charging') {
            headerSubtitle += ' \u2022 Charging inhibited';
        } else if (batteryStatus === 'Full') {
            headerSubtitle += ' \u2022 Full';
        }

        this.menu.setHeader(this.gicon, 'Power', headerSubtitle);
    }
