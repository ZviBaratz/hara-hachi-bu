...
class PowerManagerToggle extends QuickSettings.QuickMenuToggle {
    constructor(settings, extensionObject, stateManager) {
        ...
        if (this.accessible) {
            this.accessible.accessible_name = _('Hara Hachi Bu - Switch power modes, battery thresholds, and scenarios');
        }
        ...
        this._inlineErrorItem.connectObject('activate', () => {
    ...
    _rebuildProfileItems() {
        ...
        if (profiles.length === 0) {
            const hint = new PopupMenu.PopupMenuItem(
                _('Create scenarios in Settings to save mode combinations'),
        ...
        for (const profile of profiles) {
            ...
            if (hasRules) {
                ...
                if (badge.accessible)
                    badge.accessible.accessible_name = _('Automatically managed scenario');
        ...
        this._saveAsScenarioItem = new PopupMenu.PopupMenuItem(
            _('Save as Scenario\u2026')
        );
        this._saveAsScenarioItem.connectObject('activate', () => {
    ...
    _addForceDischargeToggle() {
        ...
        this._forceDischargeItem = new PopupMenu.PopupSwitchMenuItem(_('Use Battery While Plugged In'), forceDischargeState);
        if (this._forceDischargeItem.accessible)
            this._forceDischargeItem.accessible.accessible_name = _('Use Battery While Plugged In: force the laptop to drain battery even when connected to AC');
    ...
    _updateForceDischargeState() {
        ...
        this._forceDischargeItem.label.text = onBattery
            ? _('Use Battery While Plugged In (AC only)')
    ...
    _addAutoManageSection() {
        ...
        if (this._autoSwitchToggle.accessible)
            this._autoSwitchToggle.accessible.accessible_name = _('Auto-switch scenarios: automatically apply scenarios when system conditions change');
    ...
    async _saveAsScenario() {
        ...
        try {
            const success = ProfileMatcher.createProfile(
                this._settings, id, name, powerMode, batteryMode
            );
    ...
    _getBatteryStatusText() {
        ...
        if (currentProfile) {
            ...
            primaryParts.push(_('No scenario'));
        }
        ...
        if (this._stateManager.boostChargeActive) {
            ...
            secondary = remaining > 0
                ? _('Boost (%s remaining)').format(this._formatTime(remaining))
                : _('Boost Charge');
        } else if (batteryStatus === 'Charging') {
            const timeToFull = this._stateManager.timeToFull;
            secondary = timeToFull > 0
                ? _('Charging (%s)').format(this._formatTime(timeToFull))
                : _('Charging');
            ...
            secondary = endThreshold > 0
                ? _('Not charging (limit: %d%%)').format(endThreshold)
                : _('Not charging');
    ...
    _updateSubtitle() {
        if (this._showingSuccessFeedback)
            return;
    ...
    _setMenuItemsSensitive(sensitive) {
        ...
        if (this._saveAsScenarioItem) {
            this._saveAsScenarioItem.sensitive = sensitive;
        }
    }

    _showInlineError(message) {
        if (!this._inlineErrorItem)
            return;
