...
class PowerManagerToggle extends QuickSettings.QuickMenuToggle {
    ...
    _buildProfileSection() {
        ...
        const headerItem = new PopupMenu.PopupSeparatorMenuItem(_('Profile'));
    ...
    _setManualOverridesExpanded(expanded) {
        ...
        if (this._manualOverridesHeader) {
            this._manualOverridesHeader.accessible_name = expanded
                ? _('Manual Overrides (expanded)')
                : _('Manual Overrides (collapsed)');
        }
    }
    ...
    _updateManualOverridesExpanded() {
        ...
        // Use simpler label when there are no profiles (nothing to "override")
        if (this._manualOverridesHeader) {
            this._manualOverridesHeader.label.text = profiles.length === 0
                ? _('Controls')
                : _('Manual Overrides');
        }
    ...
    async _cycleProfile() {
        ...
        if (!profiles || profiles.length === 0) {
            Main.notify(_('Unified Power Manager'), _('No profiles configured. Open Extension Settings to create profiles.'));
            return;
        }
        ...
        const nextProfileId = profiles[nextIndex];

        const profile = ProfileMatcher.getProfileById(this._settings, nextProfileId);
        const profileName = profile ? profile.name : nextProfileId;

        this._stateManager.pauseAutoManage();
        this._setLoadingState(true, _('Applying %s...').format(profileName));
        try {
            await this._stateManager.setProfile(nextProfileId);
        } finally {
            this._setLoadingState(false);
        }
    }
    ...
    _getBatteryStatusText() {
        ...
        const parts = [`${batteryLevel}%`];

        if (batteryStatus === 'Charging') {
            let chargingText = _('Charging');
            const timeToFull = this._stateManager.timeToFull;
            if (timeToFull > 0)
                chargingText += ` (${this._formatTime(timeToFull)})`;
            parts.push(chargingText);
        } else if (batteryStatus === 'Discharging') {
            let dischargingText = this._stateManager.forceDischargeEnabled
                ? _('Force discharging') : _('Discharging');
            const timeToEmpty = this._stateManager.timeToEmpty;
            if (timeToEmpty > 0)
                dischargingText += ` (${this._formatTime(timeToEmpty)})`;
            parts.push(dischargingText);
        } else if (batteryStatus === 'Not charging') {
            parts.push(_('Not charging (threshold reached)'));
