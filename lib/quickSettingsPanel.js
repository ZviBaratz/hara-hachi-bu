...
class PowerManagerToggle extends QuickSettings.QuickMenuToggle {
    constructor(settings, extensionObject, stateManager) {
        ...
        this._noProfilesFeedbackTimeoutId = null;
    ...
    _rebuildProfileItems() {
        ...
        // Add "Manual" option for when modes don't match any profile
        this._customProfileItem = new PopupMenu.PopupMenuItem(_('Manual'));
    ...
    _buildBatteryModeItems() {
        ...
        for (const mode of batteryModes) {
            ...
            item.connectObject('activate', async () => {
                ...
                this._setLoadingState(true, _('Setting %s...').format(_(config.label)));
                try {
                    // Manual mode change - this will pause auto-management
                    this._stateManager.pauseAutoManage();
    ...
    _scheduleBatteryModeRebuild() {
        ...
        this._batteryModeRebuildTimeout = GLib.timeout_add(GLib.PRIORITY_DEFAULT, 50, () => {
            this._batteryModeRebuildTimeout = null;
    ...
    async _cycleProfile() {
        ...
        if (!profiles || profiles.length === 0) {
            this._setLoadingState(true, _('No profiles configured'));
            this._noProfilesFeedbackTimeoutId = GLib.timeout_add(GLib.PRIORITY_DEFAULT, 1500, () => {
                this._noProfilesFeedbackTimeoutId = null;
                this._setLoadingState(false);
                return GLib.SOURCE_REMOVE;
            });
        ...
        this._stateManager.pauseAutoManage();
        this._setLoadingState(true, _('Switching to %s...').format(profileName));
        try {
    ...
    destroy() {
        ...
        if (this._noProfilesFeedbackTimeoutId) {
            GLib.Source.remove(this._noProfilesFeedbackTimeoutId);
            this._noProfilesFeedbackTimeoutId = null;
        }
        if (this._batteryModeRebuildTimeout) {
            GLib.Source.remove(this._batteryModeRebuildTimeout);
