...
class PowerManagerToggle extends QuickSettings.QuickMenuToggle {
    ...
    _buildBatteryModeSection() {
        ...
        if (this._stateManager.batteryNeedsHelper) {
            const item = new PopupMenu.PopupMenuItem(_('Setup Required (Click to Copy Install Command)'));
            item.connectObject('activate', () => {
                ...
                Main.notify(
                    ...
                    _('Install command copied!\n\nPaste in terminal to enable battery threshold control.')
                );
            }, this);
    ...
    _getBatteryStatusText() {
        ...
        // Add health if available and not 100%
        if (health !== null && health < 95) {
            statusText += _(' \u2022 Health: %d%%').format(health);
        }
