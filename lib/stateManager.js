...
const SETUP_REQUIRED_TITLE = _('Setup Required');
const SETUP_REQUIRED_MESSAGE = _('Helper script not installed. Open Settings \u2192 About to copy installation command.');
...
export const StateManager = GObject.registerClass({
            ...
            if (desiredState !== currentState) {
                ...
                if (!success) {
                    ...
                    if (this._batteryController.needsHelper) {
                        this._notifyError(SETUP_REQUIRED_TITLE, SETUP_REQUIRED_MESSAGE);
                    } else {
                        this._notifyError(
                            _('State Restoration'),
    ...
    async _evaluateAndApplyRules() {
        if (this._destroyed || !this._parameterDetector || this._evaluatingRules || this._autoManagePaused) {
            return;
        }
    ...
    async setProfile(profileName, isAuto = false) {
        ...
        // Now run _updateProfile once with both modes applied
        this._updateProfile();
        // Emit state-changed so the UI re-renders with the correct profile ornament
        this.emit('state-changed');
