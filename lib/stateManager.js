...
export const StateManager = GObject.registerClass({
    ...
    async initialize() {
        ...
        this._initializeLoginManagerProxy();
        ...
        if (success && isAuto) {
            ...
            if (now - lastTime >= AUTO_SWITCH_NOTIFICATION_THROTTLE_MS) {
                ...
                if (profile.schedule?.enabled) {
                    ...
                    if (endTime)
                        message = _('Switched to %s (until %s)').format(getProfileDisplayName(profile), endTime);
                }
                Main.notify(
    ...
    async activateBoostCharge() {
        ...
        Main.notify(
            _('Unified Power Manager'),
            _('Boost charge activated — charging to 100%')
        );
