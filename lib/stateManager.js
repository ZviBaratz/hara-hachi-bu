...
export const StateManager = GObject.registerClass({
        ...
        if (this._batteryController && this._batteryController.supportsForceDischarge) {
            ...
            if (desiredState !== currentState) {
                ...
                if (desiredState && this._batteryController.onBattery) {
                    console.debug('Unified Power Manager: Skipping force-discharge restore (on battery)');
                } else {
        ...
        if (this._batteryController) {
            this._batteryController.connectObject(
                ...
                'battery-status-changed', () => {
                    ...
                    if (this._boostChargeActive && this._batteryController.batteryLevel >= 100) {
                        this._deactivateBoostCharge('battery_full').catch(e => {
                            if (!this._destroyed)
                                console.error(`Unified Power Manager: Error deactivating boost charge: ${e.message}`);
                        });
                    }
                    this.emit('state-changed');
                },
    ...
    _onParameterChanged(paramName, paramValue) {
        ...
        if (this._boostChargeActive && paramName === 'power_source' && paramValue === 'battery') {
            this._deactivateBoostCharge('ac_disconnected').catch(e => {
                if (!this._destroyed)
                    console.error(`Unified Power Manager: Error deactivating boost charge on AC disconnect: ${e.message}`);
            });
        }

        // Re-apply force discharge when AC reconnects, if current profile specifies it
        if (paramName === 'power_source' && paramValue === 'ac') {
            this._reapplyForceDischargeIfNeeded().catch(e => {
                if (!this._destroyed)
                    console.error(`Unified Power Manager: Error re-applying force discharge: ${e.message}`);
            });
        }
    ...
    /**
     * Re-apply force discharge from the current profile when AC reconnects.
     * When a profile with forceDischarge:'on' is applied on battery, force discharge
     * is skipped. This method applies it once AC power returns.
     * @private
     */
    async _reapplyForceDischargeIfNeeded() {
        if (this._destroyed || !this._currentProfile)
            return;

        const config = ProfileMatcher.getProfileById(this._settings, this._currentProfile);
        if (!config || config.forceDischarge !== 'on')
            return;

        if (!this._batteryController?.supportsForceDischarge)
            return;

        // Already enabled — nothing to do
        if (this._batteryController.forceDischargeEnabled)
            return;

        console.debug('Unified Power Manager: AC reconnected, re-applying force discharge from current profile');
        const success = await this._batteryController.setForceDischarge(true, false);
        if (!this._destroyed && success)
            this._settings.set_boolean('force-discharge-enabled', true);
    }
    ...
    async _applyProfile(profile, isAuto = false) {
        ...
        if (success && isAuto) {
            ...
            if (now - lastTime >= AUTO_SWITCH_NOTIFICATION_THROTTLE_MS) {
                ...
                let message = _('Switched to %s profile').format(getProfileDisplayName(profile));
    ...
    _rescheduleTimer() {
        ...
        for (const profile of profiles) {
            if (!profile.autoManaged || !profile.schedule?.enabled)
                continue;
        ...
        this._scheduleTimerId = GLib.timeout_add_seconds(GLib.PRIORITY_DEFAULT, delay, () => {
            ...
            if (!this._destroyed) {
                if (!this._autoManagePaused)
                    this._scheduleRuleEvaluation();
                // Always reschedule even when paused so timer is ready when unpaused
                this._rescheduleTimer();
            }
            return GLib.SOURCE_REMOVE;
        });
            ...
            this._prepareForSleepId = this._loginManagerProxy.connectSignal(
                ...
                (_proxy, _sender, [starting]) => {
                    ...
                    if (!starting) {
                        ...
                        console.debug('Unified Power Manager: Resumed from suspend, rescheduling timer');
                        this._rescheduleTimer();
                        if (!this._autoManagePaused && this._settings.get_boolean('auto-switch-enabled'))
                            this._scheduleRuleEvaluation();
                    }
                }
            );
        } catch (e) {
            console.debug(`Unified Power Manager: Could not connect to login manager: ${e.message}`);
        }
    }
    ...
    async activateBoostCharge() {
        ...
        this._boostChargeTimeoutId = GLib.timeout_add_seconds(GLib.PRIORITY_DEFAULT, timeoutHours * 3600, () => {
            ...
            if (!this._destroyed && this._boostChargeActive) {
                this._deactivateBoostCharge('timeout').catch(e => {
                    if (!this._destroyed)
                        console.error(`Unified Power Manager: Error on boost charge timeout: ${e.message}`);
                });
            }
            return GLib.SOURCE_REMOVE;
        });
        ...
        Main.notify(
            _('Unified Power Manager'),
            _('Boost Charge activated — charging to 100% (auto-stops in %d hours or when full)').format(timeoutHours)
        );
    ...
    async _deactivateBoostCharge(reason) {
        ...
        if (!savedPaused && autoSwitchEnabled) {
            ...
            if (savedMode && this._batteryController?.canControlThresholds) {
                await this.setBatteryMode(savedMode, false);
                if (this._destroyed) return false;
            }
