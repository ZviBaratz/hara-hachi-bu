...
const SETUP_REQUIRED_MESSAGE = N_(
    'Helper script not installed. Open Settings \u2192 About to copy installation command.'
);
    ...
    class StateManager extends GObject.Object {
        ...
        async initialize() {
            ...
            if (this._batteryController && this._batteryController.canControlThresholds) {
                ...
                if (detectedMode) this._currentBatteryMode = detectedMode;
            }
                ...
                this._batteryController.connectObject(
                    ...
                    (controller, start, end) => {
                        ...
                        if (detectedMode && detectedMode !== this._currentBatteryMode) {
                            ...
                            // Skip profile detection during boost charge to avoid flicker
                            // (boost thresholds 95/100 may match full-capacity mode)
                            if (!this._boostChargeActive) this._updateProfile();
                    ...
                    'partial-failure',
                    (controller, succeededBat, failedBats) => {
                        this._notifyError(
                            _('Partial Failure'),
                    ...
                    if (this._settings.get_boolean('auto-switch-enabled')) {
                        if (this._autoManagePaused) {
                            ...
                            if (
                                ...
                                console.debug(
                                    'Hara Hachi Bu: Profile saved with active schedule, resuming auto-management'
                                );
                                this._setAutoManagePaused(false);
                            }
                        }
                        if (!this._autoManagePaused) this._scheduleRuleEvaluation();
                    }
                },
            ...
            if (this._settings.get_boolean('auto-switch-enabled') && !this._autoManagePaused) {
                ...
                this._initialRuleEvalTimeout = GLib.timeout_add(
                    GLib.PRIORITY_DEFAULT,
                    INITIAL_RULE_EVAL_DELAY_MS,
                    () => {
                        this._initialRuleEvalTimeout = null;
                        this._evaluateAndApplyRules().catch((e) => {
                            if (!this._destroyed) console.error(`Hara Hachi Bu: Error evaluating rules: ${e.message}`);
        ...
        async _initializeParameterDetector() {
            if (this._parameterDetector) return;
        ...
        _onParameterChanged(paramName, paramValue) {
            ...
            if (!this._settings.get_boolean('auto-switch-enabled')) return;
        ...
        _scheduleRuleEvaluation() {
            if (this._ruleEvaluationTimeout) GLib.Source.remove(this._ruleEvaluationTimeout);
        ...
        async _evaluateAndApplyRules() {
            if (this._destroyed || !this._parameterDetector || this._autoManagePaused) return;
            ...
            try {
                ...
                if (matchingProfile) {
                    ...
                    if (matchingProfile.id !== this._currentProfile) {
                        console.debug(`Hara Hachi Bu: Rule matched profile "${matchingProfile.name}"`);
                ...
                this._evaluatingRules = false;
                if (!this._destroyed) this._rescheduleTimer();
         ...
         * @private
         */
        _evaluateAndApplyRulesForSchedule() {
            if (this._destroyed || !this._parameterDetector) return;
            if (!this._settings.get_boolean('auto-switch-enabled')) return;
            // Don't interfere with active boost charge
            if (this._boostChargeActive) return;

            // Schedule boundaries override manual pause
            if (this._autoManagePaused) {
                console.debug('Hara Hachi Bu: Schedule boundary reached, resuming auto-management');
                this._setAutoManagePaused(false);
            }
            ...
            if (success && isAuto) {
                ...
                if (now - lastTime >= AUTO_SWITCH_NOTIFICATION_THROTTLE_MS) {
                    ...
                    if (profile.schedule?.enabled) {
                        ...
                        if (endTime) causes.push(_('schedule until %s').format(endTime));
                        else causes.push(_('schedule active'));
                    }
                    if (profile.rules && profile.rules.length > 0) {
                        const ruleDescs = profile.rules
                            .map((rule) => {
                                ...
                                if (param.type === 'numeric') {
                                    ...
                                    valueLabel = param.valueLabels?.[rule.value]
                                        ? _(param.valueLabels[rule.value])
                                        : rule.value;
                                }
                            ...
                            .filter(Boolean);
                        if (ruleDescs.length > 0) causes.push(ruleDescs.join(', '));
                        else causes.push(_('conditions matched'));
                    }

                    if (causes.length > 0)
                        message = _('Switched to %s (%s)').format(getProfileDisplayName(profile), causes.join(', '));

                    Main.notify(_('Hara Hachi Bu'), message);
                }
            }
         ...
         */
        _setAutoManagePaused(paused, reason = 'manual') {
            if (this._destroyed) return;
            if (this._autoManagePaused !== paused) {
                this._autoManagePaused = paused;
                ...
                this.emit('auto-manage-paused-changed', paused);
                console.debug(`Hara Hachi Bu: Auto-manage ${paused ? 'paused' : 'resumed'}`);

                // Notify user when auto-switching is paused by manual action
                // (not during boost charge, which manages pause state internally)
                if (paused && reason === 'manual') {
                    const now = Date.now();
                    const lastTime = this._lastNotifications.get('auto-manage-paused') ?? 0;
                    if (now - lastTime >= AUTO_SWITCH_NOTIFICATION_THROTTLE_MS) {
                        this._lastNotifications.set('auto-manage-paused', now);
                        Main.notify(_('Hara Hachi Bu'), _('Auto-switching paused. Open the Power menu to resume.'));
                    }
                }
            }
        }
        ...
        _notifyError(title, message) {
            ...
            if (this._lastNotifications.size > 20) {
                for (const [k, t] of this._lastNotifications) {
                    if (now - t > NOTIFICATION_PRUNE_AGE_MS) this._lastNotifications.delete(k);
                }
            }
        ...
        get hasStartThreshold() {
            if (this._batteryController) return this._batteryController.hasStartThreshold;
            return false;
        }
        ...
        get currentEndThreshold() {
            if (this._batteryController) return this._batteryController.currentEndThreshold;
            return -1;
        }
        ...
        async setPowerMode(mode, notifyError = true) {
            if (!this._powerController || !this._powerController.isAvailable || this._destroyed) {
                if (!this._destroyed && notifyError) {
                    this._notifyError(
                        _('Power Mode'),
                        _('Power profile control is not available. Install power-profiles-daemon.')
                    );
                }
                return false;
            }
            ...
            try {
                ...
                if (success) {
                    ...
                    if (notifyError) {
                        this._notifyError(
                            _('Power Mode'),
                            _('Failed to apply power mode %s. Check that power-profiles-daemon is running.').format(
                                mode
                            )
                        );
                    }
        ...
        async setBatteryMode(mode, notifyError = true) {
            ...
            if (!thresholds) {
                if (notifyError) this._notifyError(_('Battery Mode'), _('Invalid battery mode: %s').format(mode));
                return false;
            }
                ...
                if (notifyError) {
                    if (this._batteryController.needsHelper) {
                        ...
                        this._notifyError(
                            _('Battery Mode'),
                            _(
                                'Failed to apply battery thresholds. Try: 1) Run install-helper.sh to set up battery control, 2) Check Settings \u2192 About for installation instructions.'
                            )
                        );
                    }
                }
         ...
         *
         * @param profileName - Profile ID to apply
         * @param isAuto - True if triggered by automatic detection (rules)
        ...
        async setProfile(profileName, isAuto = false) {
            ...
            console.debug(
                `Hara Hachi Bu: Switching to profile '${profileName}' (power: ${config.powerMode}, battery: ${config.batteryMode}, auto: ${isAuto})`
            );

            // If this is a manual switch and auto-switch is enabled, pause auto-management
            if (!isAuto && this._settings.get_boolean('auto-switch-enabled')) this._setAutoManagePaused(true);
            ...
            try {
                ...
                if (this._powerController && this._powerController.isAvailable) {
                    ...
                    console.debug(
                        `Hara Hachi Bu: Power profile controller not available, skipping power mode for profile '${profileName}'`
                    );
                }
                ...
                if (this._batteryController && this._batteryController.canControlThresholds) {
                    ...
                    console.debug(
                        `Hara Hachi Bu: Battery threshold controller not available, skipping battery mode for profile '${profileName}'`
                    );
                }
        ...
        async setForceDischarge(enabled, notifyError = true) {
            ...
            if (success) {
                ...
                if (notifyError) this._notifyError(_('Force Discharge'), _('Failed to change force discharge mode'));
                this.emit('state-changed'); // Revert UI
            }
            return success;
        }
        ...
        get forceDischargeEnabled() {
            if (this._batteryController) return this._batteryController.forceDischargeEnabled;
            return false;
        }

        get supportsForceDischarge() {
            if (this._batteryController) return this._batteryController.supportsForceDischarge;
            return false;
        }

        get batteryLevel() {
            if (this._batteryController) return this._batteryController.batteryLevel;
            return 0;
        }

        get batteryHealth() {
            if (this._batteryController) return this._batteryController.batteryHealth;
            return null;
        }

        get timeToEmpty() {
            if (this._batteryController) return this._batteryController.timeToEmpty;
            return 0;
        }

        get timeToFull() {
            if (this._batteryController) return this._batteryController.timeToFull;
            return 0;
        }

        get onBattery() {
            if (this._batteryController) return this._batteryController.onBattery;
            return false;
        }

        getBatteryStatus() {
            if (this._batteryController) return this._batteryController.getBatteryStatus();
            return 'Unknown';
        }
        ...
        _rescheduleTimer() {
            ...
            if (this._destroyed || !this._settings?.get_boolean('auto-switch-enabled')) return;

            const profiles = getCustomProfiles(this._settings);
            let minSeconds = Infinity;

            for (const profile of profiles) {
                if (!isAutoManaged(profile) || !profile.schedule?.enabled) continue;
                const secs = ScheduleUtils.secondsUntilNextBoundary(profile.schedule);
                if (secs < minSeconds) minSeconds = secs;
            }

            if (minSeconds === Infinity) return; // No scheduled profiles

            // Cap at 1 hour to guard against clock drift / DST transitions
            const delay = Math.max(1, Math.min(minSeconds, 3600));

            console.debug(`Hara Hachi Bu: Schedule timer set for ${delay}s`);
                ...
                this._prepareForSleepId = this._loginManagerProxy.connectSignal(
                    ...
                    (_proxy, _sender, [starting]) => {
                        ...
                        if (!starting) {
                            ...
                            console.debug('Hara Hachi Bu: Resumed from suspend, rescheduling timer');
                            this._rescheduleTimer();
                            if (this._settings.get_boolean('auto-switch-enabled')) {
                                this._evaluateAndApplyRulesForSchedule().catch((e) => {
                                    if (!this._destroyed)
        ...
        async activateBoostCharge() {
            if (this._destroyed || this._boostChargeActive) return false;

            if (!this._batteryController || !this._batteryController.canControlThresholds) return false;

            // Save current state
            this._boostChargeSavedBatteryMode = this._currentBatteryMode;
            this._boostChargeSavedAutoManagePaused = this._autoManagePaused;

            // Pause auto-management (suppress notification — boost manages its own UI feedback)
            this._setAutoManagePaused(true, 'boost');
            ...
            this.emit('state-changed');
            Main.notify(_('Hara Hachi Bu'), _('Boost Charge activated — charging to 100%. Auto-stops when full.'));
         ...
         *
         * @param reason - Why boost was deactivated: 'user', 'battery_full', 'timeout', 'ac_disconnected'
        ...
        async _deactivateBoostCharge(reason) {
            ...
            Main.notify(_('Hara Hachi Bu'), messages[reason] || _('Boost charge deactivated'));
         ...
         */
        toggleBoostCharge() {
            if (this._boostChargeActive) return this._deactivateBoostCharge('user');
        ...
        get boostChargeEndTime() {
            if (!this._boostChargeActive || !this._boostChargeActivatedTime) return null;
        ...
        async refreshState() {
            if (this._batteryController) await this._batteryController.refreshValues();
