...
export const StateManager = GObject.registerClass({
    ...
    async initialize() {
        ...
        if (this._batteryController) {
            this._batteryController.connectObject(
                ...
                'power-source-changed', () => {
                    // Handled by ParameterDetector via rule-based system
                    this.emit('state-changed');
                },
        ...
        this._settings.connectObject(
            ...
            // Legacy profile settings: These trigger profile updates for users upgrading
            // from older versions. The actual migration happens in extension.js via
            // migrateProfilesToCustomFormat(). These watchers can be removed in v2.0+.
            ...
            // Legacy feature flags: These settings were replaced by the rule-based profile
            // switching system (auto-switch-enabled + custom-profiles with rules).
            // We keep these watchers as no-ops to prevent errors if users have these
            // settings stored from older versions. The prefs UI no longer shows these
            // options. These watchers can be removed in v2.0+.
            'changed::docking-detection-enabled', () => {
                // No-op: Replaced by rule-based auto-switching
            },
        ...
        if (this._settings.get_boolean('auto-switch-enabled') && !this._autoManagePaused) {
            ...
            this._initialRuleEvalTimeout = GLib.timeout_add(GLib.PRIORITY_DEFAULT, 500, () => {
                this._initialRuleEvalTimeout = null;
                this._evaluateAndApplyRules();
    ...
    async setBatteryMode(mode, updateOverrideFlag = true) {
        if (!this._batteryController || !this._batteryController.isAvailable || this._destroyed) {
            if (!this._destroyed)
