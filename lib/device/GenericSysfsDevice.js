...
export const GenericSysfsDevice = GObject.registerClass({
    ...
    async initialize() {
        ...
        // The polkit policy hardcodes /usr/local/bin/unified-power-ctl as the authorized
        ...
        if (!this.ctlPath) {
            console.warn(`Unified Power Manager: ${Constants.HELPER_BIN_NAME} helper not found, battery control will be read-only`);
    ...
    async _onThresholdChangedFromMonitor() {
        try {
            ...
            if (newEnd === null) {
                console.debug(`Unified Power Manager: File monitor fired but could not read ${this.endPath}`);
                return;
            }
    ...
    async setThresholds(startValue, endValue) {
        ...
        if (!validateThreshold(start) || !validateThreshold(end)) {
            console.error(`Unified Power Manager: Invalid threshold values: start=${start}, end=${end}`);
            return false;
        }

        if (this._hasStartThreshold && start >= end)
            return false;
        ...
        try {
            if (this._hasStartThreshold) {
                ...
                [status, stdout, stderr] = await runCommandCtl(this.ctlPath, cmd, `${end}`, `${start}`);
            } else {
                // Only set end threshold
                [status, stdout, stderr] = await runCommandCtl(this.ctlPath, `${this.batteryName}_END`, `${end}`);
            }
            ...
            if (status === exitCode.SUCCESS) {
                ...
                console.error(`Unified Power Manager: Helper script not found at ${this.ctlPath}. Run install-helper.sh`);
            } else if (stderr) {
            ...
            if (!this._destroyed) {
                ...
                if (!signalEmitted && !this._destroyed) {
                    ...
                    if (!this._destroyed) {
                        ...
                        if (!this._destroyed && currentEnd !== null && (currentEnd !== this.endLimitValue ||
    ...
    async setForceDischarge(enabled) {
        ...
        if (this._settingForceDischarge) {
            console.debug('Unified Power Manager: setForceDischarge already in progress, skipping');
            return false;
        }
        ...
        try {
            ...
            if (!validateForceDischargeMode(mode)) {
                 console.error(`Unified Power Manager: Invalid force discharge mode '${mode}'`);
                 return false;
            }
            ...
            if (status === exitCode.SUCCESS) {
                ...
                for (let i = 0; i < FORCE_DISCHARGE_VERIFY_DELAYS_MS.length; i++) {
                    ...
                    await new Promise(resolve => {
                        ...
                        this._forceDischargeVerifyTimeout = GLib.timeout_add(GLib.PRIORITY_DEFAULT, FORCE_DISCHARGE_VERIFY_DELAYS_MS[i], () => {
                            this._forceDischargeVerifyTimeout = null;
                            this._forceDischargeVerifyResolve = null;
                            resolve();
                ...
                console.error('Unified Power Manager: Privilege required for force discharge — polkit rules may not be configured');
            } else if (status === exitCode.COMMAND_NOT_FOUND) {
                console.error(`Unified Power Manager: Helper script not found at ${this.ctlPath}. Run install-helper.sh`);
            } else if (stderr) {
