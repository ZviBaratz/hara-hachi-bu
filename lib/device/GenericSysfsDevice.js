...
export const GenericSysfsDevice = GObject.registerClass({
    ...
    constructor(batteryPath) {
        ...
        // Validate battery name immediately
        if (!validateBatteryName(batteryName)) {
            console.warn(`Unified Power Manager: Invalid battery name '${batteryName}', using safe default 'BAT0' for display but keeping path.`);
            this.batteryName = 'BAT0'; // Fallback for safety, though path is used for ID
        } else {
    ...
    async setThresholds(startValue, endValue) {
        ...
        if (!validateBatteryName(this.batteryName)) {
            console.error(`Unified Power Manager: Invalid battery name '${this.batteryName}'`);
            return false;
        }
            ...
            if (status === exitCode.SUCCESS) {
                ...
                for (let i = 0; i < delays.length; i++) {
                    ...
                    await new Promise(resolve => {
                        GLib.timeout_add(GLib.PRIORITY_DEFAULT, delays[i], () => {
                            resolve();
