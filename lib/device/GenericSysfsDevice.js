...
export const GenericSysfsDevice = GObject.registerClass({
     ...
     */
    getHealth() {
        // Try energy_* first (Wh), then charge_* (Ah)
        let full = readFileInt(this.energyFullPath);
        let design = readFileInt(this.energyFullDesignPath);

        if (full === null || design === null) {
            full = readFileInt(this.chargeFullPath);
            design = readFileInt(this.chargeFullDesignPath);
        }

        if (full !== null && design !== null && design > 0) {
            let health = Math.round((full / design) * 100);
