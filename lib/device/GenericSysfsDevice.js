...
export const GenericSysfsDevice = GObject.registerClass({
    ...
    constructor(batteryPath) {
        ...
        this.endPath = `${batteryPath}/charge_control_end_threshold`;
        this.startPath = `${batteryPath}/charge_control_start_threshold`;
        this.capacityPath = `${batteryPath}/capacity`;
        this.forceDischargePath = `${batteryPath}/charge_behaviour`;
        
        // Health related paths
        this.energyFullDesignPath = `${batteryPath}/energy_full_design`;
        this.energyFullPath = `${batteryPath}/energy_full`;
        this.chargeFullDesignPath = `${batteryPath}/charge_full_design`;
        this.chargeFullPath = `${batteryPath}/charge_full`;
    ...
    static isSupported(batteryPath) {
        ...
        const endPath = `${batteryPath}/charge_control_end_threshold`;
     ...
     *
     * CRITICAL: Thresholds must be written in the correct order to avoid kernel errors:
     * - When increasing thresholds (new start >= current end): write END first, then START
     * - When decreasing thresholds: write START first, then END
     * - This prevents invalid intermediate states where start > end
     *
     * Example: Changing from [60-80] to [40-70]:
     * - Write START=40 first (now 40-80), then END=70 (now 40-70) ✓
     * - If END=70 first (now 60-70), then START=40 would fail because 60>70 temporarily ✗
     *
    ...
    async setForceDischarge(enabled) {
        ...
        if (status === exitCode.SUCCESS) {
            // Verify state change with short polling to account for kernel latency
            for (let i = 0; i < 3; i++) {
            ...
            this.refreshValues(); // Re-read actual state from disk
