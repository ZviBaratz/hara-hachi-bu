...
export const GenericSysfsDevice = GObject.registerClass({
    ...
    async initialize() {
        ...
        this._hasStartThreshold = fileExists(BAT0_START_PATH);
    ...
    static isSupported() {
        ...
        return fileExists(BAT0_END_PATH);
    }
        ...
        this._monitorLevelId = this._monitorLevel.connect('changed', (obj, theFile, otherFile, eventType) => {
            if (eventType === Gio.FileMonitorEvent.CHANGES_DONE_HINT) {
                ...
                if (this._hasStartThreshold) {
                    newStart = readFileInt(BAT0_START_PATH);
                }
    ...
    async setThresholds(startValue, endValue) {
        ...
        if (this._hasStartThreshold) {
            ...
            [status] = await runCommandCtl(this.ctlPath, 'BAT0_END', `${endValue}`);
        }
