...
export const GenericSysfsDevice = GObject.registerClass({
    ...
    async initialize() {
        ...
        if (!this.ctlPath) {
            console.warn(`Hara Hachi Bu: ${Constants.HELPER_BIN_NAME} helper not found, battery control will be read-only`);
    ...
    async _onThresholdChangedFromMonitor() {
        try {
            ...
            if (!this._destroyed)
                console.error(`Hara Hachi Bu: File monitor error: ${e.message}`);
        }
    }
    ...
    async setThresholds(startValue, endValue) {
        ...
        try {
            ...
            if (status === exitCode.SUCCESS) {
                ...
                console.error('Hara Hachi Bu: Privilege required - polkit rules may not be configured. Run install-helper.sh');
    ...
    async setForceDischarge(enabled) {
        ...
        try {
            ...
            if (status === exitCode.SUCCESS) {
                ...
                console.warn(`Hara Hachi Bu: Force discharge write succeeded but verification failed. Reverting state.`);
                await this.refreshValues(); // Re-read actual state from disk
                this.emit('force-discharge-changed', this._forceDischargeEnabled);
                return false;
            } else if (status === exitCode.PRIVILEGE_REQUIRED) {
                console.error('Hara Hachi Bu: Privilege required for force discharge — polkit rules may not be configured');
            } else if (status === exitCode.COMMAND_NOT_FOUND) {
                console.error(`Hara Hachi Bu: Helper script not found at ${this.ctlPath}. Run install-helper.sh`);
            } else if (stderr) {
