...
export const MockDevice = GObject.registerClass({
    ...
    async setThresholds(start, end) {
        console.log(`Unified Power Manager: MockDevice: Setting thresholds to ${start}-${end}`);
        if (start < 0 || end > 100 || start >= end) {
             console.log('Unified Power Manager: MockDevice: Invalid thresholds');
             return false;
        }
    ...
    async setForceDischarge(enabled) {
        console.log(`Unified Power Manager: MockDevice: Setting force discharge to ${enabled}`);
