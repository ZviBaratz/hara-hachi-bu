...
import {debugLog} from '../helper.js';
...
export const MockDevice = GObject.registerClass(
    ...
    class MockDevice extends BaseDevice {
        ...
        initialize() {
            debugLog('Initializing Mock Device');
            return true;
        }
        ...
        setThresholds(start, end) {
            debugLog('MockDevice: Setting thresholds to', start + '-' + end);
            if (start < 0 || end > 100 || start >= end) {
                debugLog('MockDevice: Invalid thresholds');
                return false;
            }
        ...
        setForceDischarge(enabled) {
            debugLog('MockDevice: Setting force discharge to', enabled);
