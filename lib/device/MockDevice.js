...
export const MockDevice = GObject.registerClass({
    ...
    async initialize() {
        console.log('Hara Hachi Bu: Initializing Mock Device');
        return true;
    }
    ...
    async setThresholds(start, end) {
        console.log(`Hara Hachi Bu: MockDevice: Setting thresholds to ${start}-${end}`);
        if (start < 0 || end > 100 || start >= end) {
             console.log('Hara Hachi Bu: MockDevice: Invalid thresholds');
             return false;
        }
    ...
    async setForceDischarge(enabled) {
        console.log(`Hara Hachi Bu: MockDevice: Setting force discharge to ${enabled}`);
