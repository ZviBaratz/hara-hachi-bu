...
export const MockDevice = GObject.registerClass(
    ...
    class MockDevice extends BaseDevice {
        ...
        initialize() {
            console.debug('Hara Hachi Bu: Initializing Mock Device');
            return true;
        }
        ...
        setThresholds(start, end) {
            console.debug(`Hara Hachi Bu: MockDevice: Setting thresholds to ${start}-${end}`);
            if (start < 0 || end > 100 || start >= end) {
                console.debug('Hara Hachi Bu: MockDevice: Invalid thresholds');
                return false;
            }
        ...
        setForceDischarge(enabled) {
            console.debug(`Hara Hachi Bu: MockDevice: Setting force discharge to ${enabled}`);
