 ...
 *
 * Uses Promise.allSettled (SpiderMonkey 78+, GNOME 40+) so that one device
 * failure does not mask results from other devices. Safe for GNOME 46+ target.
 */
    ...
    class CompositeDevice extends BaseDevice {
        ...
        async setThresholds(start, end) {
            ...
            if (failedSecondary.length > 0) {
                ...
                debugWarn('CompositeDevice setThresholds failed for secondary batteries:', failedNames);
        ...
        async setForceDischarge(enabled) {
            ...
            if (failedOthers.length > 0) {
                ...
                debugWarn('CompositeDevice setForceDischarge failed for:', failedNames);
        ...
        async refreshValues() {
            ...
            for (let i = 0; i < results.length; i++) {
                if (results[i].status === 'rejected') {
                    debugError('refreshValues failed for', this._devices[i].batteryName + ':', results[i].reason);
                }
            }
        }
