...
export const CompositeDevice = GObject.registerClass({
    ...
    async setThresholds(start, end) {
        ...
        if (failedSecondary.length > 0) {
            ...
            console.warn(`Hara Hachi Bu: CompositeDevice setThresholds failed for secondary batteries: ${failedNames}`);
    ...
    async refreshValues() {
        ...
        for (let i = 0; i < results.length; i++) {
            if (results[i].status === 'rejected')
                console.error(`Hara Hachi Bu: refreshValues failed for ${this._devices[i].batteryName}: ${results[i].reason}`);
        }
    }
