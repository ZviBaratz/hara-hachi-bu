...
export const CompositeDevice = GObject.registerClass({
    ...
    async setThresholds(start, end) {
        ...
        // Log any secondary battery failures
        const failedSecondary = this._devices.filter((d, i) => i > 0 && results[i] !== true);
        if (failedSecondary.length > 0) {
            console.warn(`Unified Power Manager: CompositeDevice setThresholds failed for secondary batteries: ${failedSecondary.map(d => d.batteryName).join(', ')}`);
        }
    ...
    async setForceDischarge(enabled) {
        ...
        if (failedOthers.length > 0) {
            console.warn(`Unified Power Manager: CompositeDevice setForceDischarge failed for: ${failedOthers.map(d => d.batteryName).join(', ')}`);
        }
