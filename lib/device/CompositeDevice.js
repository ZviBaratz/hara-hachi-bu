...
export const CompositeDevice = GObject.registerClass({
        ...
        this._devices.forEach((device, index) => {
            device.connectObject(
                ...
                'force-discharge-changed', (dev, enabled) => {
                    ...
                    const isFirstSupported = dev.supportsForceDischarge &&
    ...
    async setThresholds(start, end) {
        ...
        if (failedSecondary.length > 0) {
            ...
            console.warn(`Unified Power Manager: CompositeDevice setThresholds failed for secondary batteries: ${failedNames}`);
    ...
    async setForceDischarge(enabled) {
        ...
        // Primary device determines overall success
        const primarySuccess = results[0] === true;
        ...
        if (failedOthers.length > 0) {
            ...
            console.warn(`Unified Power Manager: CompositeDevice setForceDischarge failed for: ${failedNames}`);
    ...
    async refreshValues() {
        ...
        for (let i = 0; i < results.length; i++) {
            if (results[i].status === 'rejected')
                console.error(`Unified Power Manager: refreshValues failed for ${this._devices[i].batteryName}: ${results[i].reason}`);
        }
    }
