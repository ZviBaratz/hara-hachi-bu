...
export const CompositeDevice = GObject.registerClass({
    ...
    async setThresholds(start, end) {
        ...
        if (!allSucceeded) {
            const failed = this._devices.filter((d, i) => results[i] !== true).map(d => d.batteryName);
            console.warn(`Unified Power Manager: CompositeDevice setThresholds failed for: ${failed.join(', ')}`);
        }
        return allSucceeded;
    }
    ...
    async setForceDischarge(enabled) {
        ...
        const allSucceeded = results.every(res => res === true);
        if (!allSucceeded) {
            const failed = targetDevices.filter((d, i) => results[i] !== true).map(d => d.batteryName);
            console.warn(`Unified Power Manager: CompositeDevice setForceDischarge failed for: ${failed.join(', ')}`);
        }
        return allSucceeded;
    }
