...
const {readFileAsync, debugError, debugWarn} = Helper;

export class DeviceManager {
    static async getDevice(cancellable = null) {
        ...
        try {
            ...
            if (!powerSupplyDir.query_exists(null)) {
                debugWarn(Constants.SYSFS_POWER_SUPPLY_PATH, 'does not exist');
                return null;
            }
            ...
            while (true) {
                ...
                const fileInfos = await enumerator.next_files_async(10, GLib.PRIORITY_DEFAULT, cancellable);
            ...
            for (const devInfo of supportedDevicesInfos) {
                ...
                try {
                    ...
                    debugError('Error initializing device', devInfo.name + ':', devError);
                } finally {
                    device?.destroy();
                }
            }
            ...
            for (const dev of initializedDevices)
                dev.destroy();
            debugError('Error enumerating power supplies:', e);
        }
