    ...
    static async getDevice() {
        ...
        try {
            ...
            if (supportedDevicesInfos.length > 0) {
                console.log(`Unified Power Manager: Found ${supportedDevicesInfos.length} supported battery device(s): ${supportedDevicesInfos.map(d => d.name).join(', ')}`);
            } else {
                console.log('Unified Power Manager: No supported battery devices found (standard sysfs)');
                return null;
            }
