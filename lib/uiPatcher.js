...
export class UIPatcher {
    ...
    hideBuiltinPowerProfile() {
        ...
        if (!found && !this._hideRetryTimeout) {
            ...
            console.debug(`Unified Power Manager: Built-in indicator not found, starting retry loop (${maxRetries} attempts)`);
    ...
    _tryHideBuiltinPowerProfile() {
        ...
        for (let i = 0; i < nItems; i++) {
            ...
            // Skip our own indicator! Check for our title 'Power' (not 'Power Mode' or 'Power Profiles')
            const toggleTitle = toggle.title || (toggle.label ? toggle.label.text : '');
            if (toggleTitle === 'Power') {
                continue;
            }
            ...
            if (toggleTitle === 'Power Mode' || toggleTitle === 'Power Profiles') {
                return this._doHide(indicator, i);
            }
            ...
            if (toggle.gicon) {
                ...
                if (iconName.includes('power-profile-balanced') ||
                    ...
                    console.debug('Unified Power Manager: Detected built-in profile via icon name');
                    return this._doHide(indicator, i);
                }
            }
        }
    ...
    _doHide(indicator, index) {
        console.debug(`Unified Power Manager: Found built-in power profile at index ${index}, hiding it`);
