...
export const PowerProfileController = GObject.registerClass(
    {
        GTypeName: 'HhbPowerProfileController',
        Signals: {
    ...
    class PowerProfileController extends GObject.Object {
        ...
        async initialize() {
            ...
            try {
                ...
                if (this._destroyed) return false;
                console.debug(`Hara Hachi Bu: D-Bus initialization failed: ${e}`);
            }
            ...
            console.debug('Hara Hachi Bu: No power profile control available');
            return false;
        }
            ...
            for (const profile of profiles) {
                ...
                if (unpacked.Profile) {
                    const profileName = unpacked.Profile.deep_unpack
                        ? unpacked.Profile.deep_unpack()
                        : unpacked.Profile;
                    this._availableProfiles.push(profileName);
                }
            }

            if (this._availableProfiles.length === 0) this._availableProfiles = ['balanced'];
        }

        async _refreshFromCli() {
            if (!this._powerprofilesctlPath || this._destroyed) return;

            const [status, stdout, stderr] = await runCommand([this._powerprofilesctlPath, 'get']);
            if (this._destroyed) return;
            if (status === 0 && stdout) this._currentProfile = stdout.trim();
            else if (stderr) console.error(`Hara Hachi Bu: powerprofilesctl get failed: ${stderr.trim()}`);
            ...
            if (listStatus === 0 && listStdout) {
                ...
                for (const line of lines) {
                    ...
                    if (match) {
                        if (!this._availableProfiles.includes(match[1])) this._availableProfiles.push(match[1]);
                    }
                }
                if (this._availableProfiles.length === 0) this._availableProfiles = ['balanced'];
            } else if (listStderr) {
                console.error(`Hara Hachi Bu: powerprofilesctl list failed: ${listStderr.trim()}`);
            }
        }
        ...
        async setProfile(profile) {
            if (!this._availableProfiles.includes(profile) || this._destroyed) return false;
            ...
            if (this._proxy) {
                ...
                try {
                    ...
                    if (this._destroyed) return false;
                    console.debug(`Hara Hachi Bu: D-Bus setProfile failed, trying CLI fallback: ${e.message}`);
                }
            }

            if (this._powerprofilesctlPath) {
                const [status, , stderr] = await runCommand([this._powerprofilesctlPath, 'set', profile]);
                if (this._destroyed) return false;
                if (status === 0) {
                    this._currentProfile = profile;
                    this.emit('power-profile-changed', profile);
                    ...
                    console.error(`Hara Hachi Bu: powerprofilesctl failed: ${stderr.trim()}`);
                }
            }
