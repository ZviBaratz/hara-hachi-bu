 ...
 * @param {string|null} behaviour - Raw content of charge_behaviour file
 * @returns {boolean} - True if force-discharge is currently active
 */
...
export async function runCommandCtl(ctlPath, command, ...args) {
    ...
    const task = _ctlQueue
        ...
        .then(() => {
            if (_execDestroyed)
                return [exitCode.ERROR, null, 'Extension disabled'];
