 ...
 */
export async function readFileAsync(path, cancellable = null) {
    try {
        const f = Gio.File.new_for_path(path);
        const [contents] = await f.load_contents_async(cancellable);
 ...
 */
export async function readFileIntAsync(path, cancellable = null) {
    const v = await readFileAsync(path, cancellable);
    if (v !== null) {
...
export function runCommandCtl(ctlPath, command, ...args) {
    ...
    if (_queueDepth >= MAX_QUEUE_DEPTH) {
        ...
        return Promise.resolve([exitCode.ERROR, null, 'Command queue full - too many pending operations']);
    }
...
export async function getAcOnlineSysfs(sysfsPath, isCancelled = null) {
    ...
    try {
        ...
        // Enumeration unavailable; return null to let caller use alternative detection
    }
