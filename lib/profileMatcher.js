...
export function getCustomProfiles(settings) {
    ...
    try {
        ...
        const invalidProfiles = profiles.filter(p => validateProfile(p) === null);
        const validProfiles = profiles.filter(p => validateProfile(p) !== null);

        // Log details about invalid profiles
        if (invalidProfiles.length > 0) {
            console.warn(`Unified Power Manager: Found ${invalidProfiles.length} invalid profile(s):`);
            for (const p of invalidProfiles) {
                console.warn(`  - Profile "${p.id || p.name || 'unknown'}": ${JSON.stringify(p).slice(0, 100)}...`);
            }
        }
        ...
        if (validProfiles.length === 0) {
            ...
            console.error('Unified Power Manager: Invalid data was:', JSON.stringify(profiles).slice(0, 500));
            saveCustomProfiles(settings, defaults);
            return defaults;
        }
...
export function updateProfile(settings, profileId, updates) {
    ...
    // Builtin profiles can change name, modes, forceDischarge, and rules (ID remains stable)
