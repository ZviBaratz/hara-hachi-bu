 ...
 * @param {number} startThreshold - Start charge threshold
...
export function validateProfile(profile) {
    ...
    if (!validBatteryModes.includes(profile.batteryMode))
        return null;
    if (typeof profile.name !== 'string' || profile.name.length === 0 || profile.name.length > 50)
        return null;
 ...
 */
export function createProfile(settings, id, name, powerMode, batteryMode) {
    const profiles = getCustomProfiles(settings);
...
export function updateProfile(settings, profileId, updates) {
    ...
    // Builtin profiles can only change modes, not name
    if (profiles[index].builtin)
        updates = {powerMode: updates.powerMode, batteryMode: updates.batteryMode};
