 ...
 *
 * @param {Object} profile - Profile object with name and builtin fields
 * @returns {string} - Translated name for builtins, raw name otherwise
 */
export function getProfileDisplayName(profile) {
    return profile.builtin ? _(profile.name) : profile.name;
}
    ...
    if (profile.schedule !== undefined && profile.schedule !== null) {
        ...
        if (profile.schedule.enabled) {
            ...
            if (!scheduleResult.valid) {
                console.warn(`Unified Power Manager: Invalid schedule in profile "${profile.id}": ${scheduleResult.error}, setting to null`);
 ...
 */
export function createProfile(settings, id, name, powerMode, batteryMode, forceDischarge = 'unspecified', rules = null, autoManaged = false, schedule = null) {
    const profiles = [...getCustomProfiles(settings)];
    ...
    if (hasRules || hasSchedule) {
        const newProfile = {id, name, powerMode, batteryMode, rules: rules || [], autoManaged, schedule};
    ...
    if (validateProfile(newProfile) === null) {
        console.warn(`Unified Power Manager: createProfile validation failed for "${id}"`);
        return false;
    }
...
export function updateProfile(settings, profileId, updates) {
    ...
    if (updates.rules !== undefined || updates.autoManaged === true || updates.schedule !== undefined) {
