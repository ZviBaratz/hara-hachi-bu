...
function _migrateToRuleBasedProfiles(settings) {
    ...
    if (dockingEnabled) {
        ...
        if (dockedProfile) {
            dockedProfile.rules.push({param: 'external_display', op: 'is', value: 'connected'});
            changed = true;
        }

        const undockedProfile = profiles.find(p => p.id === undockedId);
        if (undockedProfile) {
            undockedProfile.rules.push({param: 'external_display', op: 'is', value: 'not_connected'});
            changed = true;
        }
    }
    ...
    if (powerSourceEnabled) {
        ...
        if (acProfile) {
            acProfile.rules.push({param: 'power_source', op: 'is', value: 'ac'});
            changed = true;
        }

        const batProfile = profiles.find(p => p.id === batId);
        if (batProfile) {
            batProfile.rules.push({param: 'power_source', op: 'is', value: 'battery'});
            changed = true;
        }
    }
