 ...
 * @returns {boolean} - True if the profile is builtin (docked or travel)
 */
 ...
 *
 * @param {Object} profile - Profile object with id and name fields
 * @returns {string} - Translated name for builtins, raw name otherwise
 */
 ...
 *
 * @param {Object} profile - Profile object with id field
 * @returns {string|null} - Icon name or null
 */
export function getProfileIcon(profile) {
    switch (profile.id) {
    case 'docked': return 'hhb-docked-symbolic';
    case 'travel': return 'hhb-travel-symbolic';
    default: return null;
    }
}
...
export function detectBatteryModeFromThresholds(startThreshold, endThreshold, settings = null) {
    ...
    for (const mode of modes) {
        ...
        if (isEndOnly) {
            ...
            if (Math.abs(endThreshold - expectedEnd) <= 2)
                return mode.name;
        } else {
...
export function getThresholdsForMode(batteryMode, settings = null) {
    ...
    if (settings) {
        ...
        if (start >= end) {
            console.warn(`Hara Hachi Bu: Invalid thresholds for ${batteryMode} (start=${start} >= end=${end}), using defaults`);
    ...
    if (profile.rules !== undefined && profile.rules !== null) {
        if (!Array.isArray(profile.rules)) {
            ...
            const validRules = profile.rules.filter((rule, i) => {
                ...
                if (!result.valid) {
                    console.warn(`Hara Hachi Bu: Dropping invalid rule ${i + 1} from profile "${profile.id}": ${result.error}`);
                    return false;
                }
                return true;
            });
    ...
    if (profile.schedule !== undefined && profile.schedule !== null) {
        ...
        if (profile.schedule.enabled) {
            ...
            if (!scheduleResult.valid) {
                console.warn(`Hara Hachi Bu: Invalid schedule in profile "${profile.id}": ${scheduleResult.error}, setting to null`);
 ...
 * @returns {boolean} - True if profile has rules or an active schedule
 */
    ...
    try {
        ...
        const validProfiles = profiles.filter(p => {
            if (validateProfile(p) === null) {
                console.warn(`Hara Hachi Bu: Dropping invalid profile (id: ${p?.id || 'unknown'}, name: ${p?.name || 'unknown'}): missing or invalid required fields`);
                return false;
            }
            return true;
        });
 ...
 * @param {Object|null} [schedule=null] - Optional time-based schedule
...
export function validateProfileInput(settings, id, name, powerMode, batteryMode, isEdit = false) {
    if (!name || name.trim().length === 0)
        return {valid: false, error: _('Scenario name is required')};

    if (name.trim().length > 50)
        return {valid: false, error: _('Scenario name too long (max 50 characters)')};

    if (!isValidProfileId(id))
        return {valid: false, error: _('Invalid scenario ID')};
    ...
    if (!isEdit) {
        ...
        if (profiles.length >= MAX_PROFILES)
            return {valid: false, error: _('Maximum scenario limit reached')};
    }
