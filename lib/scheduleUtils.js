...
export function schedulesOverlap(schedule1, schedule2) {
    ...
    if (isOvernight1) {
        ...
        // Check if schedule2 is active on those "next days"
        for (const day of schedule1.days) {
            const nextDay = day === 7 ? 1 : day + 1;
            if (schedule2.days.includes(nextDay)) {
                // Schedule1 morning [0, e1) vs schedule2 intervals on nextDay
                if (_intervalsOverlap([[0, e1Min]], intervals2))
                    return true;
            }
        }
    }
    ...
    if (isOvernight2) {
        for (const day of schedule2.days) {
            ...
            if (schedule1.days.includes(nextDay)) {
                if (_intervalsOverlap(intervals1, [[0, e2Min]]))
                    return true;
            }
        }
    }
