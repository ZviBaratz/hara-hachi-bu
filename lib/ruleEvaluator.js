...
export function findMatchingProfile(profiles, currentParams) {
    try {
        ...
        for (const profile of profiles) {
            ...
            if (!profile.autoManaged || (!hasRules && !hasSchedule))
                continue;
            ...
            const rulesMatch = hasRules ? evaluateRules(profile.rules, currentParams) : true;
            if (!rulesMatch)
                continue;
...
export function findRuleConflict(profiles, newProfile, editingProfileId = null) {
    ...
    for (const existing of profiles) {
        ...
        // The ONLY thing that prevents a conflict at same specificity
        // is both profiles having non-overlapping schedules
        if (existingHasSchedule && newHasSchedule) {
            if (!ScheduleUtils.schedulesOverlap(existing.schedule, newProfile.schedule))
                continue; // Non-overlapping schedules: safe, no conflict
        }

        // All other cases: neither scheduled, only one scheduled,
        // or both scheduled with overlap → conflict
        return existing;
    }
